...
class Functions(private val rpc: NodeRPCConnection){
    ...
    fun connectionState(): List<StateAndRef<ConnectionState>>{
        return rpc.proxy.vaultQuery(ConnectionState::class.java).states
    }

    fun participantState(): List<StateAndRef<ParticipantState>>{
        return rpc.proxy.vaultQuery(ParticipantState::class.java).states
    }
    ...
    fun returnConnectionState(flowReturn: FlowHandle<SignedTransaction>): ConnectionState{
        return flowReturn.returnValue.get().coreTransaction.outputStates.first() as ConnectionState
    }

    fun returnParticipantState(flowReturn: FlowHandle<SignedTransaction>): ParticipantState{
        return flowReturn.returnValue.get().coreTransaction.outputStates.first() as ParticipantState
    }
}